...
src = 'D:/Ku/2.0/2.5/duLieu'
# 1. Lấy danh sách tên file
fileS_List = os.listdir(src) 
# 2. Kiểm tra file lai.txt có tồn tại không và xoá nó để chuẩn bị ghi thông tin
...
# 4. Duyệt từng file trong list file dữ liệu
for x in files_file:
    # 4.1. Tách lấy ngày đang duyệt
    y = x.split(".")
    today = y[0]

    # 4.2. Tạo các biến cần thiết ---------------------------------
    Date = x
    motTramSo = []
    mang = [x for x in range(100)]
